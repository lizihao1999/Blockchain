...
{
    deanon_t* new=deanon_new();
    block_t* temp=bc->blocks;
    while (temp!=NULL) {
        trans_t* temp2=temp->trans;
        while (temp2!=NULL) {
            add_associations(temp2->inputs,temp2->num_inputs,new);
            temp2=temp2->next;
        }
        temp=temp->next;
    }
...
double cluster_balance(cluster_t* cluster, blockchain_t* bc)
{
    ...
    while (temp!=NULL) {
        double a=0;
        double* t=&a;
        int j=get_balance(temp->addr,t,bc->addr_bal);
        if (j!=0) {
            i=i+*t;
        }
        temp=temp->next;
